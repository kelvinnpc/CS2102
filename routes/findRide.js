...
var getRideInfo_query = 'With tempTable as ' +
	...
	`tempTable.did<>$1 and now()<date and numSeats<>0 and status='open' order by date desc`;
			...
			pool.query(getTotalBids_query, [req.user.nric], (err2, getTotalBids) => {
				pool.query(isDriver_query, [req.user.nric], (err3, driverCheck) => {
					if (driverCheck.rows[0].count == 0)
						basic(req, res, 'findRide', { title: 'Available rides', driver: false, getRideInfo: getRideInfo.rows, 
							getWalletBalance: getWalletBalance.rows, getTotalBids: getTotalBids.rows });
					else
						basic(req, res, 'findRide', { title: 'Available rides', driver: true, getRideInfo: getRideInfo.rows, getWalletBalance: getWalletBalance.rows, getTotalBids: getTotalBids.rows });
